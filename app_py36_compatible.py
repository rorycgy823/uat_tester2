...
# -*- coding: utf-8 -*-
"""
CDSW-Only Phi-2 UAT Generator (Python 3.6 Compatible)
======================================================

This script has been tuned to run in a Python 3.6 environment.

CRITICAL: Before running, install the required packages using:
...
class CDSWPhi2Model(object):
    """
    Pure CDSW Phi-2 GGUF model loader.
    (Using object for Python 2/3 compatibility style).
    """
    ...
    def _load_phi2_model(self):
        ...
        try:
            logger.info("🚀 Loading Phi-2 GGUF model for CDSW (Python 3.6)...")
            
            # This import will now work if you have installed the correct requirements
            ...
            for path in model_paths:
                if os.path.exists(path):
                    model_path = path
                    logger.info("📁 Found GGUF model: {}".format(path))
                    break
            
            if not model_path:
                raise FileNotFoundError("❌ No phi-2.Q4_K_M.gguf found in CDSW paths")
            
            # In older ctransformers versions, explicit model_type is often crucial.
            # The 'phi' type might not exist; 'gpt2' or 'llama' are safer bets.
            model_types = ["gpt2", "llama", "phi"]
            
            for model_type in model_types:
                try:
                    logger.info("🧠 Loading with model_type: {}".format(model_type))
                    ...
                    logger.info("✅ Phi-2 model loaded successfully with type: {}".format(model_type))
                    self.model_loaded = True
                    return
                    
                except Exception as e:
                    logger.warning("⚠️ Model type '{}' failed: {}".format(model_type, e))
                    continue
            
            raise Exception("❌ All model types failed")
            
        except ImportError:
            logger.error("❌ ctransformers not installed. Run: pip install -r requirements_py36.txt")
            raise
        except Exception as e:
            logger.error("❌ Phi-2 model loading failed: {}".format(e))
            raise
    
    def generate_uat(self, user_story):
...
""".format(user_story)
    ...
    def _extract_test_cases(self, text):
    ...
    def _extract_configuration(self, text):
...
security_settings:
  ...
  audit_logging: true"""
    ...
    def _calculate_quality_score(self, test_cases, config):
...
<body>
    ...
    <script>
        function generateUAT() {
            var userStory = document.getElementById('user_story').value;
            if (!userStory.trim()) {
...
def generate_uat():
    try:
        ...
        logger.info("🔄 Generating UAT for: {}...".format(user_story[:50]))
        
        # Generate UAT with Phi-2
        result = phi2_model.generate_uat(user_story)
        
        logger.info("✅ UAT generated successfully (Quality: {}%)".format(result['quality_score']))
        return jsonify(result)
        
    except Exception as e:
        logger.error("❌ UAT generation failed: {}".format(e))
...
if __name__ == '__main__':
    ...
    logger.info("🚀 Starting CDSW Phi-2 UAT Generator on {}:{}".format(host, port))
